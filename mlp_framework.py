...
class layer:
    ...
    '''
    def __init__(self,in_dim,nodes=32): 

        #activation and weights
        ## IF no_bias != True: in_dim+1 = in_dim (delete the 1)
        self.f = f_relu
        self.w = np.random.uniform(-.1,.1,(nodes,in_dim+1))
...
class dueling_mlp:
    ...
    - not converging so far - think it's about target_updates:
      ...
      dueling Q update:  Q_target = reward + gamma * 1/number_of_actions * mean of what are you talking about (maybe all Q action values?) ?!
